...
with st.form("inspection_form"):
    ...
    label_name = "JOH/Clearance" if point_type == "TWS" else "Clearance"
...
if not st.session_state.data:
    ...
    for i, rec in enumerate(st.session_state.data):
        ...
        with st.expander(
            ...
            with c1:
                st.write(f"LH Opening : {rec['LH Opening']}")
                st.write(f"LH Housing : {rec['LH Housing']}")
            ...
            with c2:
                st.write(f"RH Opening : {rec['RH Opening']}")
                st.write(f"RH Housing : {rec['RH Housing']}")
            ...
            with ec1:
                ...
                edit_lh_open = st.number_input(
                    "Edit LH Opening",
                    value=int(rec['LH Opening']),
                    step=1,
                    key=f"lho_{i}"
                )

                edit_lh_housing = st.number_input(
                    "Edit LH Housing",
                    value=int(rec['LH Housing']),
                    step=1,
                    key=f"lhh_{i}"
                )
            ...
            with ec2:
                ...
                edit_rh_open = st.number_input(
                    "Edit RH Opening",
                    value=int(rec['RH Opening']),
                    step=1,
                    key=f"rho_{i}"
                )

                edit_rh_housing = st.number_input(
                    "Edit RH Housing",
                    value=int(rec['RH Housing']),
                    step=1,
                    key=f"rhh_{i}"
                )
            ...
            for loc, values in rec['Gauge_Level'].items():
                ...
                gauge_edit = gc1.text_input(
                    f"Edit Gauge {loc}",
                    value=values['gauge'],
                    key=f"g_{loc}_{i}"
                )

                level_edit = gc2.text_input(
                    f"Edit Level {loc}",
                    value=values['level'],
                    key=f"l_{loc}_{i}"
                )

                edited_gauge_level[loc] = {
                    'gauge': gauge_edit,
                    'level': level_edit
                }
            ...
            if st.button(
                ...
                rec['Point No'] = edit_point

                rec['LH Opening'] = edit_lh_open
                rec['LH Housing'] = edit_lh_housing

                rec['RH Opening'] = edit_rh_open
                rec['RH Housing'] = edit_rh_housing

                rec['Gauge_Level'] = edited_gauge_level

                rec['Remarks'] = edit_remarks
